// ar-camera-element.js
// This is a Wix Custom Element - upload to your Wix site's Public folder
// Then register it in your page code

const AR_PLANTS = [
  { name: 'Lanceleaf Coreopsis', acr: 'COR', img: 'https://static.wixstatic.com/media/94bd1f_48d31eb0d72044c5a798b2fd8960256a~mv2.png' },
  { name: 'Blazingstar', acr: 'LIA', img: 'https://static.wixstatic.com/media/94bd1f_b53a5d6e4fb042c585a424bea9356928~mv2.png' },
  { name: 'Purple Lovegrass', acr: 'ERA', img: 'https://static.wixstatic.com/media/94bd1f_935a7b7f12d14b1c9d71cb3122f0c1c4~mv2.png' },
  { name: 'Black-Eyed Susan', acr: 'RUD', img: 'https://static.wixstatic.com/media/94bd1f_e5d5fdb0948b4808afaf856a5020ff13~mv2.png' },
  { name: 'Purple Coneflower', acr: 'ECH', img: 'https://static.wixstatic.com/media/94bd1f_229936c40de64ae18bf2640a08e1b58a~mv2.png' },
  { name: 'Rough Goldenrod', acr: 'SOL', img: 'https://static.wixstatic.com/media/94bd1f_78ed4dc45454484f818e215a6b795305~mv2.png' },
  { name: 'Smooth Aster', acr: 'AST', img: 'https://static.wixstatic.com/media/94bd1f_6390f6e095084528ac9eb4e7e8917f2a~mv2.png' },
  { name: 'Blunt Mountain-Mint', acr: 'MMT', img: 'https://static.wixstatic.com/media/94bd1f_c877a7e1e24143ce91317966460be77b~mv2.png' },
  { name: 'Butterfly Weed', acr: 'ASC', img: 'https://static.wixstatic.com/media/94bd1f_00213b64feff49f591a7fb83fd720fbb~mv2.png' }
];

class ArCameraElement extends HTMLElement {
  constructor() {
    super();
    this.attachShadow({ mode: 'open' });
    this.stream = null;
    this.facingMode = 'environment';
    this.selectedTray = null;
    this.selectedPlant = null;
    this.placedPlants = [];
  }

  connectedCallback() {
    this.render();
    this.setupEventListeners();
  }

  disconnectedCallback() {
    if (this.stream) {
      this.stream.getTracks().forEach(t => t.stop());
    }
  }

  render() {
    this.shadowRoot.innerHTML = `
      <style>
        :host {
          display: block;
          width: 100%;
          height: 100%;
          position: relative;
          background: #000;
          overflow: hidden;
        }
        
        * { margin: 0; padding: 0; box-sizing: border-box; }
        
        #cameraView { position: absolute; inset: 0; background: #111; }
        #cameraVideo { width: 100%; height: 100%; object-fit: cover; }
        #plantsOverlay { position: absolute; inset: 0; pointer-events: none; }
        
        .arPlant {
          position: absolute;
          pointer-events: auto;
          touch-action: none;
          cursor: grab;
          filter: drop-shadow(0 4px 8px rgba(0,0,0,0.4));
          transition: transform 0.1s;
        }
        .arPlant.dragging { cursor: grabbing; transform: scale(1.1); z-index: 1000; }
        .arPlant.selected { outline: 3px solid #4CAF50; outline-offset: 3px; border-radius: 8px; }
        .arPlant img { width: 100%; height: 100%; object-fit: contain; pointer-events: none; }
        
        #topBar {
          position: absolute; top: 0; left: 0; right: 0;
          padding: 12px 15px;
          padding-top: max(12px, env(safe-area-inset-top));
          background: linear-gradient(to bottom, rgba(0,0,0,0.6), transparent);
          display: flex; justify-content: space-between; align-items: center;
          z-index: 10;
        }
        #topBar h3 { color: #fff; font-size: 16px; font-weight: 600; margin: 0; font-family: -apple-system, BlinkMacSystemFont, sans-serif; }
        
        .topBtn {
          background: rgba(255,255,255,0.2);
          backdrop-filter: blur(10px);
          -webkit-backdrop-filter: blur(10px);
          border: none; color: #fff;
          width: 36px; height: 36px;
          border-radius: 50%; font-size: 18px; cursor: pointer;
        }
        .topBtn:active { background: rgba(255,255,255,0.4); }
        
        #bottomControls {
          position: absolute; bottom: 0; left: 0; right: 0;
          padding: 12px 15px;
          padding-bottom: max(12px, env(safe-area-inset-bottom));
          background: linear-gradient(to top, rgba(0,0,0,0.8), transparent);
          z-index: 10;
        }
        
        #plantTray {
          display: flex; gap: 8px; overflow-x: auto;
          padding: 8px 0; margin-bottom: 12px;
          -webkit-overflow-scrolling: touch;
        }
        #plantTray::-webkit-scrollbar { display: none; }
        
        .trayPlant {
          flex-shrink: 0; width: 55px; height: 65px;
          background: rgba(255,255,255,0.15);
          backdrop-filter: blur(10px);
          -webkit-backdrop-filter: blur(10px);
          border-radius: 10px;
          display: flex; flex-direction: column;
          align-items: center; justify-content: center;
          cursor: pointer;
          border: 2px solid transparent;
          transition: all 0.2s;
        }
        .trayPlant:active { transform: scale(0.95); }
        .trayPlant.selected { border-color: #4CAF50; background: rgba(76,175,80,0.3); }
        .trayPlant img { width: 35px; height: 35px; object-fit: contain; }
        .trayPlant span { color: #fff; font-size: 8px; margin-top: 3px; font-family: -apple-system, BlinkMacSystemFont, sans-serif; }
        
        #actionBtns { display: flex; justify-content: center; gap: 12px; }
        
        .actionBtn {
          background: rgba(255,255,255,0.2);
          backdrop-filter: blur(10px);
          -webkit-backdrop-filter: blur(10px);
          border: none; color: #fff;
          padding: 10px 20px; border-radius: 20px;
          font-size: 13px; font-weight: 600; cursor: pointer;
          font-family: -apple-system, BlinkMacSystemFont, sans-serif;
        }
        .actionBtn:active { transform: scale(0.95); }
        .actionBtn.primary { background: #4CAF50; }
        
        #sizeControls {
          position: absolute; bottom: 150px; left: 50%;
          transform: translateX(-50%);
          display: none; gap: 8px;
          background: rgba(0,0,0,0.7);
          backdrop-filter: blur(10px);
          -webkit-backdrop-filter: blur(10px);
          padding: 6px 12px; border-radius: 20px;
          z-index: 10;
        }
        #sizeControls.vis { display: flex; }
        
        .sizeBtn {
          background: rgba(255,255,255,0.2);
          border: none; color: #fff;
          width: 32px; height: 32px;
          border-radius: 50%; font-size: 16px; cursor: pointer;
        }
        .sizeBtn:active { background: rgba(255,255,255,0.4); }
        .deleteBtn { background: rgba(244,67,54,0.8) !important; }
        
        #sizeLabel {
          color: #fff; font-size: 12px;
          min-width: 40px; text-align: center; line-height: 32px;
          font-family: -apple-system, BlinkMacSystemFont, sans-serif;
        }
        
        #hint {
          position: absolute; top: 70px; left: 50%;
          transform: translateX(-50%);
          background: rgba(0,0,0,0.7); color: #fff;
          padding: 8px 16px; border-radius: 16px;
          font-size: 12px; z-index: 10;
          opacity: 0; transition: opacity 0.3s;
          pointer-events: none; white-space: nowrap;
          font-family: -apple-system, BlinkMacSystemFont, sans-serif;
        }
        #hint.show { opacity: 1; }
        
        #permission {
          position: absolute; inset: 0;
          background: linear-gradient(135deg, #1B5E20, #4CAF50);
          display: flex; align-items: center; justify-content: center;
          z-index: 20;
          font-family: -apple-system, BlinkMacSystemFont, sans-serif;
        }
        #permission.hide { display: none; }
        
        .permContent { text-align: center; padding: 30px; max-width: 320px; }
        .permIcon { font-size: 60px; margin-bottom: 20px; }
        .permContent h3 { color: #fff; font-size: 24px; margin: 0 0 12px; }
        .permContent p { color: rgba(255,255,255,0.9); font-size: 14px; margin: 0 0 30px; line-height: 1.5; }
        
        .startBtn {
          background: #fff; color: #1B5E20;
          border: none; padding: 16px 40px;
          border-radius: 30px; font-size: 16px; font-weight: 700;
          cursor: pointer; margin-bottom: 15px;
          display: block; width: 100%;
          box-shadow: 0 4px 15px rgba(0,0,0,0.2);
        }
        .startBtn:active { transform: scale(0.98); }
        
        #errorMsg {
          color: #ffcdd2; font-size: 13px;
          margin-top: 20px; line-height: 1.4;
        }
        
        #flash {
          position: absolute; inset: 0;
          background: #fff; opacity: 0;
          pointer-events: none; z-index: 100;
          transition: opacity 0.1s;
        }
        #flash.flash { opacity: 1; }
      </style>
      
      <div id="permission">
        <div class="permContent">
          <div class="permIcon">🌿</div>
          <h3>AR Garden Preview</h3>
          <p>See how your native plants will look in your actual yard! Point your camera at the area where you want to plant.</p>
          <button class="startBtn" id="startBtn">📷 Start Camera</button>
          <div id="errorMsg"></div>
        </div>
      </div>
      
      <div id="cameraView">
        <video id="cameraVideo" autoplay playsinline muted></video>
      </div>
      
      <div id="plantsOverlay"></div>
      
      <div id="sizeControls">
        <button class="sizeBtn" id="shrinkBtn">−</button>
        <span id="sizeLabel">100%</span>
        <button class="sizeBtn" id="growBtn">+</button>
        <button class="sizeBtn deleteBtn" id="deleteBtn">🗑</button>
      </div>
      
      <div id="hint">Select a plant, then tap to place</div>
      
      <div id="topBar">
        <button class="topBtn" id="closeBtn">✕</button>
        <h3>🌿 AR Garden</h3>
        <button class="topBtn" id="flipBtn">🔄</button>
      </div>
      
      <div id="bottomControls">
        <div id="plantTray"></div>
        <div id="actionBtns">
          <button class="actionBtn" id="clearBtn">🗑 Clear</button>
          <button class="actionBtn primary" id="captureBtn">📸 Capture</button>
        </div>
      </div>
      
      <div id="flash"></div>
    `;
    
    this.buildTray();
  }

  $(id) {
    return this.shadowRoot.getElementById(id);
  }

  buildTray() {
    const tray = this.$('plantTray');
    tray.innerHTML = '';
    AR_PLANTS.forEach((p, i) => {
      const div = document.createElement('div');
      div.className = 'trayPlant';
      div.innerHTML = `<img src="${p.img}" alt="${p.name}"><span>${p.acr}</span>`;
      div.onclick = () => this.selectTrayPlant(i, div);
      tray.appendChild(div);
    });
  }

  setupEventListeners() {
    this.$('startBtn').onclick = () => this.startCamera();
    this.$('flipBtn').onclick = () => this.flipCamera();
    this.$('closeBtn').onclick = () => this.close();
    this.$('clearBtn').onclick = () => this.clearAll();
    this.$('captureBtn').onclick = () => this.capture();
    this.$('growBtn').onclick = () => this.resizePlant(1.2);
    this.$('shrinkBtn').onclick = () => this.resizePlant(0.8);
    this.$('deleteBtn').onclick = () => this.deletePlant();
    
    this.$('plantsOverlay').onclick = (e) => {
      if (e.target !== this.$('plantsOverlay')) return;
      if (this.selectedTray !== null) {
        this.placePlant(e.clientX, e.clientY);
      } else if (this.selectedPlant) {
        this.selectedPlant.classList.remove('selected');
        this.selectedPlant = null;
        this.$('sizeControls').classList.remove('vis');
      }
    };
  }

  showHint(msg) {
    const hint = this.$('hint');
    hint.textContent = msg;
    hint.classList.add('show');
    setTimeout(() => hint.classList.remove('show'), 2500);
  }

  async startCamera() {
    try {
      if (this.stream) {
        this.stream.getTracks().forEach(t => t.stop());
      }
      
      this.stream = await navigator.mediaDevices.getUserMedia({
        video: {
          facingMode: this.facingMode,
          width: { ideal: 1920 },
          height: { ideal: 1080 }
        }
      });
      
      this.$('cameraVideo').srcObject = this.stream;
      this.$('permission').classList.add('hide');
      this.showHint('Select a plant and tap to place it');
      
    } catch (err) {
      console.error('Camera error:', err);
      this.$('errorMsg').innerHTML = 'Camera access denied.<br>Please allow camera access in your browser settings.';
    }
  }

  async flipCamera() {
    this.facingMode = this.facingMode === 'environment' ? 'user' : 'environment';
    await this.startCamera();
    this.showHint(this.facingMode === 'environment' ? 'Back camera' : 'Front camera');
  }

  close() {
    if (this.stream) {
      this.stream.getTracks().forEach(t => t.stop());
    }
    this.dispatchEvent(new CustomEvent('close'));
  }

  selectTrayPlant(index, el) {
    if (this.selectedPlant) {
      this.selectedPlant.classList.remove('selected');
      this.selectedPlant = null;
      this.$('sizeControls').classList.remove('vis');
    }
    
    this.shadowRoot.querySelectorAll('.trayPlant').forEach(t => t.classList.remove('selected'));
    
    if (this.selectedTray === index) {
      this.selectedTray = null;
    } else {
      this.selectedTray = index;
      el.classList.add('selected');
      this.showHint('Tap anywhere to place ' + AR_PLANTS[index].name);
    }
  }

  placePlant(x, y) {
    const plant = AR_PLANTS[this.selectedTray];
    const size = 80;
    const rect = this.getBoundingClientRect();
    
    const el = document.createElement('div');
    el.className = 'arPlant';
    el.style.width = size + 'px';
    el.style.height = size + 'px';
    el.style.left = (x - rect.left - size/2) + 'px';
    el.style.top = (y - rect.top - size/2) + 'px';
    el.dataset.scale = 1;
    el.innerHTML = `<img src="${plant.img}" alt="${plant.name}">`;
    
    this.$('plantsOverlay').appendChild(el);
    this.makeDraggable(el);
    
    this.placedPlants.push(el);
    this.showHint(plant.name + ' placed!');
  }

  makeDraggable(el) {
    let startX, startY, origLeft, origTop;
    let isDragging = false;
    
    el.onpointerdown = (e) => {
      e.preventDefault();
      isDragging = true;
      el.classList.add('dragging');
      el.setPointerCapture(e.pointerId);
      
      startX = e.clientX;
      startY = e.clientY;
      origLeft = parseFloat(el.style.left) || 0;
      origTop = parseFloat(el.style.top) || 0;
    };
    
    el.onpointermove = (e) => {
      if (!isDragging) return;
      el.style.left = (origLeft + e.clientX - startX) + 'px';
      el.style.top = (origTop + e.clientY - startY) + 'px';
    };
    
    el.onpointerup = (e) => {
      isDragging = false;
      el.classList.remove('dragging');
      el.releasePointerCapture(e.pointerId);
    };
    
    el.onclick = (e) => {
      e.stopPropagation();
      this.selectPlacedPlant(el);
    };
  }

  selectPlacedPlant(el) {
    this.selectedTray = null;
    this.shadowRoot.querySelectorAll('.trayPlant').forEach(t => t.classList.remove('selected'));
    
    if (this.selectedPlant) this.selectedPlant.classList.remove('selected');
    
    this.selectedPlant = el;
    el.classList.add('selected');
    
    const scale = parseFloat(el.dataset.scale) || 1;
    this.$('sizeLabel').textContent = Math.round(scale * 100) + '%';
    this.$('sizeControls').classList.add('vis');
  }

  resizePlant(factor) {
    if (!this.selectedPlant) return;
    let scale = parseFloat(this.selectedPlant.dataset.scale) || 1;
    scale = Math.max(0.3, Math.min(3, scale * factor));
    
    const baseSize = 80;
    const newSize = baseSize * scale;
    
    const cx = parseFloat(this.selectedPlant.style.left) + parseFloat(this.selectedPlant.style.width)/2;
    const cy = parseFloat(this.selectedPlant.style.top) + parseFloat(this.selectedPlant.style.height)/2;
    
    this.selectedPlant.dataset.scale = scale;
    this.selectedPlant.style.width = newSize + 'px';
    this.selectedPlant.style.height = newSize + 'px';
    this.selectedPlant.style.left = (cx - newSize/2) + 'px';
    this.selectedPlant.style.top = (cy - newSize/2) + 'px';
    
    this.$('sizeLabel').textContent = Math.round(scale * 100) + '%';
  }

  deletePlant() {
    if (!this.selectedPlant) return;
    this.selectedPlant.remove();
    this.placedPlants = this.placedPlants.filter(p => p !== this.selectedPlant);
    this.selectedPlant = null;
    this.$('sizeControls').classList.remove('vis');
    this.showHint('Plant removed');
  }

  clearAll() {
    if (this.placedPlants.length === 0) {
      this.showHint('No plants to clear');
      return;
    }
    this.placedPlants.forEach(p => p.remove());
    this.placedPlants = [];
    if (this.selectedPlant) {
      this.selectedPlant = null;
      this.$('sizeControls').classList.remove('vis');
    }
    this.showHint('All plants cleared');
  }

  async capture() {
    if (this.selectedPlant) {
      this.selectedPlant.classList.remove('selected');
      this.selectedPlant = null;
    }
    this.$('sizeControls').classList.remove('vis');
    
    this.$('flash').classList.add('flash');
    setTimeout(() => this.$('flash').classList.remove('flash'), 150);
    
    try {
      const video = this.$('cameraVideo');
      const canvas = document.createElement('canvas');
      canvas.width = video.videoWidth || 1920;
      canvas.height = video.videoHeight || 1080;
      const ctx = canvas.getContext('2d');
      
      ctx.drawImage(video, 0, 0, canvas.width, canvas.height);
      
      const rect = this.getBoundingClientRect();
      const scaleX = canvas.width / rect.width;
      const scaleY = canvas.height / rect.height;
      
      for (const plant of this.placedPlants) {
        const img = plant.querySelector('img');
        const plantRect = plant.getBoundingClientRect();
        const relLeft = plantRect.left - rect.left;
        const relTop = plantRect.top - rect.top;
        
        await new Promise((resolve) => {
          const tempImg = new Image();
          tempImg.crossOrigin = 'anonymous';
          tempImg.onload = () => {
            ctx.drawImage(tempImg, 
              relLeft * scaleX, 
              relTop * scaleY, 
              plantRect.width * scaleX, 
              plantRect.height * scaleY
            );
            resolve();
          };
          tempImg.onerror = resolve;
          tempImg.src = img.src;
        });
      }
      
      const link = document.createElement('a');
      link.download = 'ecoplantia-ar-' + Date.now() + '.png';
      link.href = canvas.toDataURL('image/png');
      link.click();
      
      this.showHint('Photo saved!');
      
    } catch (e) {
      console.error('Capture failed:', e);
      this.showHint('Capture failed - try screenshot');
    }
  }
}

customElements.define('ar-camera-element', ArCameraElement);

...
const GardenDesigner = dynamic(() => import('@/components/GardenDesigner'), { ssr: false });

type Step = 'upload' | 'boundary' | 'existing' | 'preferences' | 'generating' | 'designer' | 'results' | 'checkout';
...
export default function Home() {
  ...
  // Step 2: Handle boundary completion from interactive editor
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      setStep('designer'); // Go to designer step instead of results
    } catch (err) {
  ...
  // Step 6: Handle mockup capture from designer
  const handleMockupCapture = async (dataUrl: string) => {
    setDesign((prev) => ({ ...prev, mockupDataUrl: dataUrl }));

    // Automatically generate beauty render
    setLoading(true);
    setError(null);
    ...
    try {
      const res = await fetch('/api/design/beauty-render', {
        ...
        body: JSON.stringify({
          jobId: design.jobId,
          mockupBase64: dataUrl,
          preset: 'ecoplantia_landscape_beauty',
        }),
      });
  ...
  // Skip to results without beauty render
  const handleSkipBeautyRender = () => {
    setStep('results');
  };
    ...
    <main style={{ minHeight: '100vh', padding: '20px' }}>
      <div className="container">
        ...
        <div style={{ display: 'flex', justifyContent: 'center', gap: '8px', marginBottom: '40px', flexWrap: 'wrap' }}>
          {(['upload', 'boundary', 'existing', 'preferences', 'generating', 'designer', 'results'] as Step[]).map((s, i) => (
            <div
              key={s}
              style={{
                padding: '6px 12px',
        ...
        {/* Step Content */}
        <div className="card" style={{ maxWidth: step === 'designer' ? '1200px' : '900px', margin: '0 auto' }}>
          ...
          {/* BOUNDARY STEP - Interactive Editor */}
          ...
          {/* DESIGNER STEP - Interactive Plant Placement */}
          {step === 'designer' && design.photoUrl && design.boundaryPx && design.pxPerIn && (
            <div>
              <h2 style={{ marginBottom: '20px', textAlign: 'center' }}>Design Your Garden</h2>
              <p style={{ textAlign: 'center', marginBottom: '20px', color: 'var(--text-secondary)' }}>
                Click to place plants, drag to reposition. Double-click to remove.
              </p>
              <GardenDesigner
                photoUrl={design.photoUrl}
                boundaryPx={design.boundaryPx}
                pxPerIn={design.pxPerIn}
                palette={design.palette.map((p: any) => ({
                  sku: p.sku,
                  name: p.name,
                  acr: p.sku.slice(0, 3).toUpperCase(),
                  size: p.size || 18,
                  img: p.img,
                ...
                onCaptureMockup={handleMockupCapture}
              />
              <div style={{ display: 'flex', gap: '10px', justifyContent: 'center', marginTop: '20px' }}>
                <button onClick={handleSkipBeautyRender} className="btn-outline">
                  Skip to Quote →
                </button>
              </div>
              {loading && (
                <div style={{ textAlign: 'center', marginTop: '20px' }}>
                  <p>Generating beauty render...</p>
                  <div className="spinner" style={{
                    width: '30px',
                    height: '30px',
          ...
          {step === 'results' && (
            <div>
              ...
              {/* Beauty Render or Mockup */}
              {design.beautyRenderUrl ? (
                <div style={{ marginBottom: '20px' }}>
                  <h3 style={{ marginBottom: '10px' }}>Beauty Render</h3>
                  <img
              ...
              ) : design.mockupDataUrl ? (
                <div style={{ marginBottom: '20px' }}>
                  <h3 style={{ marginBottom: '10px' }}>Your Mockup</h3>
                  <img
                    src={design.mockupDataUrl}
                    alt="Garden mockup"
                    style={{ width: '100%', borderRadius: '12px' }}
                  />
                </div>
                ...
                <div style={{ textAlign: 'center', marginBottom: '20px' }}>
                  <button onClick={() => setStep('designer')} className="btn-secondary">
                    ← Back to Designer
                  </button>
                </div>
              )}
              ...
              <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap' }}>
                <button onClick={() => setStep('designer')} className="btn-outline">
                  ← Edit Design
                </button>
